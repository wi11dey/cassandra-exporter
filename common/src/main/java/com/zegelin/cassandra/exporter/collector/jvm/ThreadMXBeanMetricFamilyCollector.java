...
public class ThreadMXBeanMetricFamilyCollector extends  MBeanGroupMetricFamilyCollector {
    private static final ObjectName THREAD_MXBEAN_QUERY = ObjectNames.create(ManagementFactory.THREAD_MXBEAN_NAME);
    ...
    public static Factory factory(final boolean perThreadTimingEnabled) {
        return mBean -> {
            if (!THREAD_MXBEAN_QUERY.apply(mBean.name))
                return null;
    ...
    public Stream<MetricFamily> collect() {
...

        {
            ...
            metricFamilies.add(new GaugeMetricFamily("cassandra_jvm_thread_count", "Current number of live threads.", null, Stream.of(
                    new NumericMetric(USER_THREAD_COUNT_LABELS, userThreadCount),
                    new NumericMetric(DAEMON_THREAD_COUNT_LABELS, daemonThreadCount)
            )));
        }

        metricFamilies.add(new GaugeMetricFamily("cassandra_jvm_threads_started_total", "Cumulative number of started threads (since JVM start).", null,
                Stream.of(new NumericMetric(null, threadMXBean.getTotalStartedThreadCount()))));
        ...
        if (perThreadTimingEnabled && threadMXBean instanceof com.sun.management.ThreadMXBean && threadMXBean.isThreadCpuTimeEnabled()) {
            ...
            metricFamilies.add(new CounterMetricFamily("cassandra_jvm_thread_cpu_time_seconds_total", "Cumulative thread CPU time (since JVM start).", null, threadCpuTimeMetrics.build()));
        }
